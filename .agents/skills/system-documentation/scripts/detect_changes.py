...
DEFAULT_IGNORES = (
    ...
    ".cache/**", "dist/**", "build/**", "coverage/**", "mkdoc-site/**",
    "docs/mkdoc-site/**", "*.pyc", "*.pyo", "*.log", ".DS_Store",
)
SENSITIVE = (".env", ".env.*", "**/.env", "**/.env.*", "*.pem", "*.key", "*.p12", "*.pfx", "*credentials*.json", "*service-account*.json")
DOC_FILES = {"docs/index.md", "docs/architecture.md", "docs/data-lineage.md", "docs/technical-review.md"}
...
def classify(path: str, rules: list[dict], map_path: str = 'docs/documentation-map.json', map_error: str | None = None) -> dict:
    ...
    if path in DOC_FILES or path == 'mkdocs.yml' or (path.startswith('docs/') and path.endswith(('.md', '.yaml', '.yml'))):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument('--map', default='docs/documentation-map.json', help='Map path relative to repo')
