...
class ClassifierTests(unittest.TestCase):
    ...
    def test_sensitive_and_map(self):
        ...
        self.assertEqual(module.classify('docs/documentation-map.json', self.rules)['decision'], 'manual_review')
...
class GitIntegrationTests(unittest.TestCase):
    def test_worktree_including_untracked(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (repo/'docs'/'documentation-map.json').write_text(json.dumps({'version': 1, 'rules': [
                {'globs': ['src/**'], 'documents': ['docs/data-lineage.md'], 'decision': 'review_ai', 'reason': 'Source semantics'}]}))
            g('add', '.')
            g('commit', '-qm', 'initial')
